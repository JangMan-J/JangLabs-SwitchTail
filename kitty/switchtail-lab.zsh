# SwitchTail kitty board launcher.
# Enable by adding this line to ~/.zshrc:
#     source ~/.config/kitty/switchtail-lab.zsh
#
#   lab              -> fzf picker over all lab boards
#   lab <name>       -> open that lab's board (e.g. `lab agent`)
#   lab switchtail-exchange -> open the exchange board (one claude line per lab)
...
lab() {
  ...
  if [ -n "${1:-}" ]; then
    ...
    sel="$(print -l "$dir"/*.kitty-session(N:t:r) \
           | fzf --prompt='lab board > ' --height=40% --layout=reverse --no-multi)"
  fi
  [ -z "$sel" ] && return 0
  f="$dir/$sel.kitty-session"
  [ -f "$f" ] || { print -u2 "lab: no session '$sel' (try: lab  |  lab switchtail-exchange)"; return 1; }
