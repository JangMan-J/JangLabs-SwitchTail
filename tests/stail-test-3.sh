...
echo "== R3: a 'switchtail-all' lab dir is reserved (won't clobber the aggregate session) =="
ws=/tmp/stail-test-ws3; rm -rf "$ws"; mkdir -p "$ws"
for n in agent switchtail-all all build good; do mkdir -p "$ws/$n" && : > "$ws/$n/.git"; done
WORKSPACE="$ws"
disc="$(_discover_labs 2>/tmp/r3warn | tr '\n' ' ')"
echo "  discovered: [$disc]"; echo "  warn: $(cat /tmp/r3warn)"
[ "$disc" = "agent good " ] && ok "switchtail-all/all/build all reserved; only agent+good kept" || no "discovery wrong: [$disc]"
...
echo "== R4: cmd_switch/cmd_cockpit reject bad names end-to-end =="
...
out="$(command ~/.local/bin/stail cockpit '../../evil' /tmp 2>&1)"; rc=$?
[ "$rc" -eq 2 ] && printf '%s' "$out" | grep -q 'invalid lab name' && ok "cockpit rejects traversal (exit 2, no claude)" || no "cockpit didn't reject traversal"
...
echo "== R2: cmd_switch raises the aggregate when a lab is live only inside 'all' =="
: > /tmp/r2-activate.log
...
kdotool() {
  case "$*" in
    ...
    "search --class ^switchtail-all$")    printf '%s\n' '{all-win}' ;; # aggregate is up
...
[ "$act" = '{all-win}' ] && ok "raises the aggregate window (no duplicate launch)" || no "did not raise aggregate: '$act'"
echo "$warn" | grep -q 'aggregate' && ok "explains it's raising the aggregate" || no "no aggregate note"

echo "== R2: a standalone window still wins over the aggregate =="
: > /tmp/r2-activate.log
kdotool() {
  case "$*" in
...
[ "$act" = '{standalone-agent}' ] && ok "standalone agent window raised (not the aggregate)" || no "raised wrong window: '$act'"
