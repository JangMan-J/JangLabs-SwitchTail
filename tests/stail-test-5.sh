...
: > /tmp/t5-launch.log; ( cmd_patch lab=claude*3 ) >/dev/null 2>&1
...
: > /tmp/t5-launch.log; ( cmd_patch lab=claude lab=agent ) >/dev/null 2>&1
...
printf '%s\n' "$s9" | grep -q -- '--var kind=shell stail line claude .* "shell"$' && ok "shell pane: --var kind=shell + line arg \"shell\"" || no "shell emit wrong"
