#!/usr/bin/env bash
# Regression tests for `stail trunk` (N parallel claude lines in one board window) against
# the REAL stail functions. Covers the session emitter (_emit_trunk_session) and the subcommand
# (cmd_trunk): count, class, per-pane hold tags, master focus, count validation + clamping, the
...
: > /tmp/trunk-launch.log
( cmd_trunk '../../evil' 2 ) >/dev/null 2>&1; [ $? -eq 2 ] && ok "traversal lab name -> exit 2" || no "traversal not rejected"
[ ! -s /tmp/trunk-launch.log ] && ok "no launch attempted for the bad name" || no "launched despite bad name"
# missing dir: a syntactically valid name with no repo dir
: > /tmp/trunk-launch.log
( cmd_trunk definitelynotalab 1 ) >/dev/null 2>&1; [ $? -eq 1 ] && ok "missing lab dir -> exit 1" || no "missing dir not guarded"
