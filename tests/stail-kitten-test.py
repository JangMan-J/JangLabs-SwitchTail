#!/usr/bin/env python3
"""Faithful unit tests for the REAL board kittens (no kitty runtime).
...
Then we import ~/.config/kitty/{swap,hold}.py unmodified and exercise them.
"""
...
swap = load('swap', '~/.config/kitty/swap.py')
hold = load('hold', '~/.config/kitty/hold.py')
...
def press(boss, t): swap.handle_result(None, None, None, boss)
def rec(boss, t): return getattr(boss, '_switchtail_swap', {}).get(t.id)

print("== SWAP (#1): involution + identity guard ==")
...
# simulate the user shuffling B(11) into the hot seat (keys MOVE dpad) and focusing it there:
...
check(1 not in getattr(bz, '_switchtail_swap', {}), "closed tab's record pruned on next press (R5)")

# ================= hold kitten (#3) =================
print("== HOLD (#3): close gated on a successful marker write ==")
class FakeWindow:
...
def run_hold(state_home, wmap, target):
...
# C3: side shell (kind=shell, no lab) -> never closed
b = run_hold(tempfile.mkdtemp(), {3: FakeWindow({'kind': 'shell'})}, 3)
